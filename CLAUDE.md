...
- **Multi-zone (regional) clusters:** Zone topology is auto-detected from worker node `topology.kubernetes.io/zone` labels. `CLUSTER_ZONE_COUNT`, `CLUSTER_REGION`, `CLUSTER_ZONES`, and `CLUSTER_MULTI_AZ` are exported by `00-config.sh`. On multi-zone clusters: (1) `CLUSTER_DESCRIPTION` includes AZ count and region; (2) `FILE_CSI_DEDUP`/`BLOCK_CSI_DEDUP` default to `false` so metro/retain SC variants are tested; (3) Pool CSI creates a `FileSharePool` in the first zone only (spec accepts a single zone); (4) `failureDomain` for custom pools is auto-detected from the `StorageCluster` status (computed by the ocs-operator from node topology). The CRUSH tree hierarchy on ROKS is typically: root → region → zone → rack → host. Multi-zone CRUSH topology needs validation on a deployed regional cluster to determine whether `failureDomain: zone` is needed for cross-AZ replica placement.
