...
./run-all.sh --scale-test --pool rep3 --skip-setup  # Auto-ramp single pool
...
This suite supports both **bare metal (BM)** and **VSI** ROKS clusters in single-zone or multi-zone (regional) configurations. Cluster type is auto-detected from worker node instance-type labels (`detect_cluster_type()` in `00-config.sh`), or can be overridden via `CLUSTER_TYPE=bm|vsi`. Zone topology is auto-detected from `topology.kubernetes.io/zone` labels (`detect_cluster_zones()`).
...
- **Failure domain and CRUSH topology:** The `failureDomain` controls how Ceph distributes replicas/chunks across the CRUSH tree to survive failures at a given level. On ODF, this is **not** a per-pool setting — it is computed cluster-wide by the **ocs-operator** from worker node topology labels and stored in `StorageCluster .status.failureDomain`. The ocs-operator then propagates it to all OOB CephBlockPools/CephFilesystems it manages. On ROKS this is typically `rack` (each worker gets its own rack bucket), not `host`. Custom pools created by `01-setup-storage-pools.sh` must match the cluster-level decision; `detect_failure_domain()` reads from the StorageCluster status (authoritative), falls back to the OOB CephBlockPool spec, then defaults to `rack`. The CRUSH tree hierarchy on ROKS is: root → region → zone → rack → host.
- **EC pool constraints:** EC pools require k+m unique failure domains. With 3 racks, only pools needing ≤3 failure domains work (rep2, rep3, ec-2-1). Pools requiring more (ec-3-1 and ec-2-2 need 4, ec-4-2 needs 6) are defined in `00-config.sh` for portability across cluster sizes but are automatically skipped when the cluster has insufficient failure domains. The feasibility check uses the CRUSH tree for non-host domains. Topology skips are logged as warnings and do not count as failures.
...
- **File/Block CSI deduplication:** Auto-discovery finds many StorageClasses, but `-metro-`, `-retain-`, `-regional*`, and `-min-iops*` variants are filtered out by default on single-zone clusters. Metro/retain produce identical I/O performance on a single-zone cluster; regional SCs use the `rfs` profile which requires IBM support allowlisting; min-iops (~100 IOPS at 150Gi) is too slow for fio benchmarks — the 4G test file write saturates the share and starves SSH. `FILE_CSI_DEDUP` and `BLOCK_CSI_DEDUP` default to `true` on single-zone and `false` on multi-zone clusters (where metro topology may affect latency). Can be overridden via env var.
...
- **CephFS CSI auth caps:** Rook only authorizes the CephFS CSI provisioner/node secrets (`rook-csi-cephfs-provisioner`, `rook-csi-cephfs-node`) for the OOB CephFilesystem at ODF initialization time. Custom CephFilesystems (e.g. `perf-test-cephfs-rep2`) require updating the Ceph auth caps for both users to include the new filesystem. Without this, PVC provisioning fails with `rados: ret=-1, Operation not permitted`. The provisioner user needs `allow rw tag cephfs metadata=<fs_name>` and the node user needs `allow rw tag cephfs *=<fs_name>` appended to their existing OSD caps. `01-setup-storage-pools.sh` handles this automatically via `update_cephfs_csi_caps()`, which also restarts the CephFS CSI controller pods to pick up the new auth.
...
- **Multi-zone (regional) clusters:** Zone topology is auto-detected from worker node `topology.kubernetes.io/zone` labels. `CLUSTER_ZONE_COUNT`, `CLUSTER_REGION`, `CLUSTER_ZONES`, and `CLUSTER_MULTI_AZ` are exported by `00-config.sh`. On multi-zone clusters: (1) `CLUSTER_DESCRIPTION` includes AZ count and region; (2) `FILE_CSI_DEDUP`/`BLOCK_CSI_DEDUP` default to `false` so metro/retain SC variants are tested; (3) Pool CSI creates a `FileSharePool` in the first zone only (spec accepts a single zone); (4) `failureDomain` for custom pools follows the cluster-level setting (see "Failure domain and CRUSH topology" above). Multi-zone CRUSH topology needs validation on a deployed regional cluster to determine whether the ocs-operator sets `failureDomain: zone` for cross-AZ replica placement.
