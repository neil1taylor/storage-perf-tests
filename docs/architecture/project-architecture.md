...
| `create_test_vm` | Render VM manifest and `oc apply` |
...
| `collect_vm_results` | Copy fio JSON from VM via `virtctl ssh` |
| `replace_fio_job` | Write new fio job file to a running VM via SSH (base64-encoded, sudo tee) |
| `restart_fio_service` | Clear old test data/results, stop/reset/start the benchmark service (non-blocking) |
...
| Function | Purpose |
|----------|---------|
| `parse_fio_json_to_csv` | Extract metrics from one fio JSON file into CSV rows via `jq` |
| `csv_header` | Output the CSV header row |
| `aggregate_results_csv` | Walk results tree and call `parse_fio_json_to_csv` for each file |
...
- Trap handler on `INT`/`TERM` to clean up running VMs on interruption
...
### K8s Resource Names
- Names are truncated to 63 characters (Kubernetes limit)
